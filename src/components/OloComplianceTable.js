...
export default function OloComplianceTable({ data, selectedMonth, selectedYear }) {
    const [mounted, setMounted] = useState(false);
    const [modalState, setModalState] = useState({
        isOpen: false,
        loading: false,
        branch: '',
        severity: '', // 'NON-GAMAS' | 'GAMAS' | 'QUALITY' | 'TOTAL'
    ...
    const targetReference = [
        { label: 'Non GAMAS', targetPct: '90%', targetHours: '4 Jam' },
        { label: 'GAMAS', targetPct: '90%', targetHours: '7 Jam' },
        { label: 'QUALITY', targetPct: '90%', targetHours: '7 Jam' },
        { label: 'TTR Comp Datin', targetPct: '90%', targetHours: 'Overall' }
    ];
    ...
    const handleOpenDetail = async (branch, severity, type, count) => {
        ...
        try {
            ...
            console.error("Gagal load OLO detail:", err);
    ...
    const handleExportExcel = () => {
        ...
        const exportData = modalState.tickets.map((t, idx) => ({
            ...
            'Branch': t.branch || '-',
            'STO': t.sto || '-',
            'Tipe OLO': t.resolvedType || t.priority || '-',
            'TTR (Jam)': t.ttr_tacc || '-',
        ...
        XLSX.utils.book_append_sheet(wb, ws, "OLO Compliance Details");
        XLSX.writeFile(wb, `OLO_${modalState.type.toUpperCase()}_${modalState.branch}_${modalState.severity}_${monthParam || 'Current'}.xlsx`);
    };
            ...
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 border-b border-[var(--border-subtle)] pb-5">
                <div>
                    <h3 className="text-lg font-black text-[var(--text-primary)] flex items-center gap-2.5 tracking-tight">
                        <span className="p-2 rounded-xl bg-teal-500/10 text-teal-600 dark:bg-teal-500/20 dark:text-teal-400">
                            <FaNetworkWired size={17} />
                        </span>
                        Tabel MTTR & Compliance (SQUAT OLO)
                    </h3>
                    <p className="text-xs text-[var(--text-muted)] mt-1 font-medium">
                        Monitoring pencapaian MTTR & SLA Tiket OLO Datin per Branch (MTD)
                    </p>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-2">
                    <span className="text-[10px] font-black text-[var(--text-muted)] uppercase tracking-wider flex items-center gap-1.5 mr-1">
                        <span className="h-2 w-2 rounded-full bg-teal-500"></span> Target:
                    </span>
                    {targetReference.map(t => (
                        <div key={t.label} className="flex items-center gap-1.5 px-3 py-1 rounded-xl bg-[var(--bg-base)] border border-[var(--border-color)] text-xs font-semibold shadow-xs">
                            <span className="text-[var(--text-secondary)] font-bold">{t.label}</span>
                            <span className="text-teal-600 dark:text-teal-400 font-black font-mono">{t.targetPct}</span>
                ...
                <table className="w-full border-collapse text-xs text-center">
                    <thead>
                        <tr className="bg-[var(--bg-base)] text-[var(--text-secondary)] border-b border-[var(--border-color)] font-extrabold text-[11px] uppercase tracking-wider">
                            ...
                            <th colSpan="5" className="py-2.5 px-4 border-b border-r border-[var(--border-color)] bg-teal-500/10 dark:bg-teal-900/20 font-black text-teal-700 dark:text-teal-300">
                                NON GAMAS (T ≤ 4 JAM)
                            </th>
                            <th colSpan="5" className="py-2.5 px-4 border-b border-r border-[var(--border-color)] bg-blue-500/10 dark:bg-blue-900/20 font-black text-blue-700 dark:text-blue-300">
                                GAMAS (T ≤ 7 JAM)
                            </th>
                            <th colSpan="5" className="py-2.5 px-4 border-b border-r border-[var(--border-color)] bg-purple-500/10 dark:bg-purple-900/20 font-black text-purple-700 dark:text-purple-300">
                                QUALITY (T ≤ 7 JAM)
                            </th>
                        ...
                        <tr className="bg-[var(--bg-base)]/80 text-[var(--text-muted)] font-bold text-[10px] uppercase tracking-wider border-b border-[var(--border-color)]">
                            {/* NON GAMAS */}
                            <th className="py-2 px-2 border-r border-[var(--border-color)] min-w-[70px]">TIKET</th>
                            <th className="py-2 px-2 border-r border-[var(--border-color)] min-w-[65px]">COMPLY</th>
                            <th className="py-2 px-2 border-r border-[var(--border-color)] min-w-[70px]">NOT COMPLY</th>
                            <th className="py-2 px-2 border-r border-[var(--border-color)] min-w-[65px]">REAL MTTR</th>
                            <th className="py-2 px-2 border-r border-[var(--border-color)] min-w-[75px]">ACHV</th>
                            ...
                            {/* TOTAL COMPLIANCE */}
                            <th className="py-2 px-2 border-r border-[var(--border-color)] min-w-[70px]">TOTAL TIKET</th>
                            <th className="py-2 px-2 border-r border-[var(--border-color)] min-w-[65px]">COMPLY</th>
                            <th className="py-2 px-2 border-r border-[var(--border-color)] min-w-[70px]">NOT COMPLY</th>
                            <th className="py-2 px-2 min-w-[80px]">TTR COMP</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-[var(--border-subtle)]">
                        {data.map((branchGroup, bIdx) => {
                            const nonGamas = branchGroup.types.find(t => t.typeKey === 'NON-GAMAS') || {};
                            const gamas = branchGroup.types.find(t => t.typeKey === 'GAMAS') || {};
                            const quality = branchGroup.types.find(t => t.typeKey === 'QUALITY') || {};
                            const total = branchGroup.total || {};

                            return (
                                <tr key={branchGroup.branch} className="hover:bg-slate-50/60 dark:hover:bg-slate-800/30 transition-colors">
                                    <td className="py-3 px-3 font-black text-[var(--text-muted)] border-r border-[var(--border-color)] text-xs">
                                        {bIdx + 1}
                                    </td>
                                    <td className="py-3 px-4 font-black text-[var(--text-primary)] border-r border-[var(--border-color)] text-sm tracking-wide">
                                        {branchGroup.branch}
                                    </td>

                                    {/* 1. NON GAMAS */}
                                    <td className="py-2.5 px-2 font-semibold text-[var(--text-primary)] border-r border-[var(--border-color)] font-mono">
                                        {nonGamas.total || 0}
                                    </td>
                                    <td className="py-2.5 px-2 border-r border-[var(--border-color)] font-mono">
                                        {nonGamas.comply > 0 ? (
                                            <button 
                                                onClick={() => handleOpenDetail(branchGroup.branch, 'NON-GAMAS', 'comply', nonGamas.comply)}
                                                className="font-bold text-emerald-600 dark:text-emerald-400 hover:bg-emerald-500/10 px-2 py-0.5 rounded transition"
                                                title="Lihat detail tiket Comply Non-GAMAS"
                                            >
                                                {nonGamas.comply}
                                            </button>
                                        ) : <span className="text-[var(--text-muted)] opacity-40">0</span>}
                                    </td>
                                    <td className="py-2.5 px-2 border-r border-[var(--border-color)] font-mono">
                                        {nonGamas.notComply > 0 ? (
                                            <button 
                                                onClick={() => handleOpenDetail(branchGroup.branch, 'NON-GAMAS', 'not_comply', nonGamas.notComply)}
                                                className="font-bold text-rose-600 dark:text-rose-400 hover:bg-rose-500/10 px-2 py-0.5 rounded transition"
                                                title="Lihat detail tiket Not Comply Non-GAMAS"
                                            >
                                                {nonGamas.notComply}
                                            </button>
                                        ) : <span className="text-[var(--text-muted)] opacity-40">0</span>}
                                    </td>
                                    <td className="py-2.5 px-2 font-mono text-[var(--text-secondary)] font-bold border-r border-[var(--border-color)]">
                                        {nonGamas.avgTtr}
                                    </td>
                                    <td className="py-2 px-2 border-r border-[var(--border-color)] font-mono">
                                        <span className={`inline-block px-2 py-0.5 rounded text-[10px] font-bold font-mono ${
                                            nonGamas.isMet 
                                            ? 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20' 
                                            : 'bg-rose-500/10 text-rose-600 dark:text-rose-400 border border-rose-500/20'
                                        }`}>
                                            {parseFloat(nonGamas.pct || 100).toFixed(2)}%
                                        </span>
                                    </td>
                                    ...
                                    <td className="py-2.5 px-2 border-r border-[var(--border-color)] font-mono">
                                        {gamas.comply > 0 ? (
                                            <button 
                                                onClick={() => handleOpenDetail(branchGroup.branch, 'GAMAS', 'comply', gamas.comply)}
                                                className="font-bold text-emerald-600 dark:text-emerald-400 hover:bg-emerald-500/10 px-2 py-0.5 rounded transition"
                                                title="Lihat detail tiket Comply GAMAS"
                                            >
                                                {gamas.comply}
                                            </button>
                                        ) : <span className="text-[var(--text-muted)] opacity-40">0</span>}
                                    ...
                                    <td className="py-2.5 px-2 border-r border-[var(--border-color)] font-mono">
                                        {gamas.notComply > 0 ? (
                                            <button 
                                                onClick={() => handleOpenDetail(branchGroup.branch, 'GAMAS', 'not_comply', gamas.notComply)}
                                                className="font-bold text-rose-600 dark:text-rose-400 hover:bg-rose-500/10 px-2 py-0.5 rounded transition"
                                                title="Lihat detail tiket Not Comply GAMAS"
                                            >
                                                {gamas.notComply}
                                            </button>
                                        ) : <span className="text-[var(--text-muted)] opacity-40">0</span>}
                                    ...
                                    <td className="py-2 px-2 border-r border-[var(--border-color)] font-mono">
                                        <span className={`inline-block px-2 py-0.5 rounded text-[10px] font-bold font-mono ${
                                            ...
                                            {parseFloat(gamas.pct || 100).toFixed(2)}%
                                        </span>
                                    </td>
                                    ...
                                    <td className="py-2.5 px-2 border-r border-[var(--border-color)] font-mono">
                                        {quality.comply > 0 ? (
                                            <button 
                                                onClick={() => handleOpenDetail(branchGroup.branch, 'QUALITY', 'comply', quality.comply)}
                                                className="font-bold text-emerald-600 dark:text-emerald-400 hover:bg-emerald-500/10 px-2 py-0.5 rounded transition"
                                                title="Lihat detail tiket Comply Quality"
                                            >
                                                {quality.comply}
                                            </button>
                                        ) : <span className="text-[var(--text-muted)] opacity-40">0</span>}
                                    ...
                                    <td className="py-2.5 px-2 border-r border-[var(--border-color)] font-mono">
                                        {quality.notComply > 0 ? (
                                            <button 
                                                onClick={() => handleOpenDetail(branchGroup.branch, 'QUALITY', 'not_comply', quality.notComply)}
                                                className="font-bold text-rose-600 dark:text-rose-400 hover:bg-rose-500/10 px-2 py-0.5 rounded transition"
                                                title="Lihat detail tiket Not Comply Quality"
                                            >
                                                {quality.notComply}
                                            </button>
                                        ) : <span className="text-[var(--text-muted)] opacity-40">0</span>}
                                    ...
                                    <td className="py-2 px-2 border-r border-[var(--border-color)] font-mono">
                                        <span className={`inline-block px-2 py-0.5 rounded text-[10px] font-bold font-mono ${
                                            ...
                                            {parseFloat(quality.pct || 100).toFixed(2)}%
                                        </span>
                                    </td>

                                    {/* 4. TOTAL TTR COMPLIANCE */}
                                    <td className="py-2.5 px-2 font-bold text-[var(--text-primary)] border-r border-[var(--border-color)] font-mono">
                                        {total.total || 0}
                                    </td>
                                    <td className="py-2.5 px-2 border-r border-[var(--border-color)] font-mono">
                                        {total.comply > 0 ? (
                                            <button 
                                                onClick={() => handleOpenDetail(branchGroup.branch, 'TOTAL', 'comply', total.comply)}
                                                className="font-black text-emerald-600 dark:text-emerald-400 hover:bg-emerald-500/10 px-2 py-0.5 rounded transition"
                                                title="Lihat detail seluruh tiket Comply OLO"
                                            >
                                                {total.comply}
                                            </button>
                                        ) : <span className="text-[var(--text-muted)] opacity-40">0</span>}
                                    ...
                                    <td className="py-2.5 px-2 border-r border-[var(--border-color)] font-mono">
                                        {total.notComply > 0 ? (
                                            <button 
                                                onClick={() => handleOpenDetail(branchGroup.branch, 'TOTAL', 'not_comply', total.notComply)}
                                                className="font-black text-rose-600 dark:text-rose-400 hover:bg-rose-500/10 px-2 py-0.5 rounded transition"
                                                title="Lihat detail seluruh tiket Not Comply OLO"
                                            >
                                                {total.notComply}
                                            </button>
                                        ) : <span className="text-[var(--text-muted)] opacity-40">0</span>}
                                    ...
                                    <td className="py-2 px-2 font-mono">
                                        <span className={`inline-block px-2.5 py-1 rounded-md text-[11px] font-black font-mono ${
                                            total.isMet
                ...
                <div className="fixed inset-0 z-[99999] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-fadeIn">
                    <div className="w-full max-w-5xl bg-[var(--bg-surface)] rounded-2xl shadow-2xl border border-[var(--border-color)] overflow-hidden flex flex-col max-h-[85vh] my-auto relative">
                            ...
                            <div>
                                <div className="flex items-center gap-2.5">
                                    <h3 className="font-bold text-lg text-white tracking-tight">
                                        Detail Tiket OLO: {modalState.branch}
                                    </h3>
                        ...
                        <div className="p-4 bg-[var(--bg-base)] border-b border-[var(--border-color)] flex items-center justify-between gap-4 shrink-0">
                            <div className="relative flex-1">
                                ...
                                <input 
                                    ...
                                    className="w-full pl-9 pr-4 py-2 text-xs bg-[var(--bg-surface)] border border-[var(--border-color)] rounded-xl text-[var(--text-primary)] focus:outline-none focus:border-teal-500 transition"
                                />
                            </div>
                        </div>
                        ...
                        <div className="p-4 overflow-y-auto flex-1 custom-scrollbar">
                            {modalState.loading ? (
                                <div className="py-20 flex flex-col items-center justify-center gap-2 text-teal-600">
