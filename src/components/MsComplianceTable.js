...
export default function MsComplianceTable({ data, selectedMonth, selectedYear }) {
    ...
    const handleOpenDetail = async (title, branch, category, subcategory, type, count) => {
        ...
        try {
            ...
            console.error("Gagal load MS detail:", err);
    ...
    const handleExportExcel = () => {
        ...
        const exportData = modalState.tickets.map((t, idx) => ({
            'No': idx + 1,
            'ID Tiket': t.id_tiket,
            'Kategori': `${t.category}${t.subcategory ? '-' + t.subcategory : ''}`,
            'Branch': t.branch || '-',
            'STO': t.sto || '-',
            'TTR (Jam)': t.ttr_tacc || '-',
            'Target SLA': '4 Jam',
            'Status SLA': t.isComply ? 'IN SLA (COMPLY)' : 'OVER SLA (NOT COMPLY)',
        ...
        XLSX.utils.book_append_sheet(wb, ws, "MS SLA Details");
        XLSX.writeFile(wb, `MS_${modalState.type.toUpperCase()}_${modalState.branch}_${modalState.title.replace(/[\s-]/g, '_')}_${monthParam || 'Current'}.xlsx`);
    };
            ...
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 border-b border-[var(--border-subtle)] pb-5">
                <div>
                    <h3 className="text-lg font-black text-[var(--text-primary)] flex items-center gap-2.5 tracking-tight">
                        ...
                        Tabel MTTR & SLA Compliance (MS-Eksternal)
                    </h3>
                    <p className="text-xs text-[var(--text-muted)] mt-1 font-medium">
                        Monitoring waktu penyelesaian tiket (TTR) dan kepatuhan SLA MS TACC per Branch (MTD)
                    </p>
                </div>

                {/* Target SLA Reference Badge */}
                <div className="flex items-center gap-2">
                    <div className="flex items-center gap-2 px-3.5 py-1.5 rounded-xl bg-[var(--bg-base)] border border-[var(--border-color)] text-xs font-semibold shadow-xs">
                        <span className="h-2 w-2 rounded-full bg-blue-500"></span>
                        <span className="text-[var(--text-secondary)] font-bold">Target SLA Seluruh MS:</span>
                        <span className="text-blue-600 dark:text-blue-400 font-black font-mono">≤ 4 Jam</span>
                    </div>
                </div>
            </div>
                ...
                <table className="w-full border-collapse text-xs text-center">
                    <thead>
                        <tr className="bg-[var(--bg-base)] text-[var(--text-secondary)] border-b border-[var(--border-color)] font-extrabold text-[11px] uppercase tracking-wider">
                            <th className="py-3 px-3 border-r border-[var(--border-color)] w-14">NO</th>
                            <th className="py-3 px-4 border-r border-[var(--border-color)] min-w-[130px]">BRANCH</th>
                            <th className="py-3 px-4 border-r border-[var(--border-color)] text-left pl-5 min-w-[160px]">KATEGORI MS</th>
                            <th className="py-3 px-3 border-r border-[var(--border-color)] min-w-[90px]">TARGET SLA</th>
                            <th className="py-3 px-3 border-r border-[var(--border-color)] min-w-[100px]">JUMLAH TIKET</th>
                            <th className="py-3 px-3 border-r border-[var(--border-color)] min-w-[100px] text-emerald-600 dark:text-emerald-400">IN SLA (COMPLY)</th>
                            <th className="py-3 px-3 border-r border-[var(--border-color)] min-w-[110px] text-rose-600 dark:text-rose-400">OVER SLA (NOT COMPLY)</th>
                            <th className="py-3 px-3 border-r border-[var(--border-color)] min-w-[100px]">REAL MTTR</th>
                            <th className="py-3 px-4 min-w-[110px]">SLA COMPLIANCE</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-[var(--border-subtle)]">
                        {data.map((branchGroup, bIdx) => (
                            <React.Fragment key={branchGroup.branch}>
                                {branchGroup.rows.map((row, rIdx) => (
                                    <tr key={row.key} className="hover:bg-slate-50/60 dark:hover:bg-slate-800/30 transition-colors">
                                        {rIdx === 0 && (
                                            <>
                                                <td 
                                                    rowSpan={branchGroup.rows.length + 1} 
                                        ...
                                        <td className="py-2.5 px-4 font-extrabold text-[var(--text-primary)] border-r border-[var(--border-color)] text-left pl-5 tracking-wide text-xs">
                                            {row.label}
                                        </td>
                                        <td className="py-2.5 px-3 border-r border-[var(--border-color)] font-semibold text-[var(--text-muted)] font-mono">
                                            ≤ 4 Jam
                                        </td>
                                        ...
                                        <td className="py-2.5 px-3 border-r border-[var(--border-color)] font-mono">
                                            {row.met > 0 ? (
                                                <button 
                                                    onClick={() => handleOpenDetail(row.label, branchGroup.branch, row.category, row.subcategory, 'comply', row.met)}
                                                    className="font-bold text-emerald-600 dark:text-emerald-400 hover:bg-emerald-500/10 px-2 py-0.5 rounded transition cursor-pointer"
                                                    title={`Lihat detail tiket In SLA ${row.label} (${branchGroup.branch})`}
                                                >
                                                    {row.met}
                                                </button>
                                        ...
                                        <td className="py-2.5 px-3 border-r border-[var(--border-color)] font-mono">
                                            {row.missed > 0 ? (
                                                <button 
                                                    onClick={() => handleOpenDetail(row.label, branchGroup.branch, row.category, row.subcategory, 'not_comply', row.missed)}
                                                    className="font-bold text-rose-600 dark:text-rose-400 hover:bg-rose-500/10 px-2 py-0.5 rounded transition cursor-pointer"
                                                    title={`Lihat detail tiket Over SLA ${row.label} (${branchGroup.branch})`}
                                                >
                                                    {row.missed}
                                                </button>
                                            ) : <span className="text-[var(--text-muted)] opacity-40">0</span>}
                                        ...
                                        {/* REAL MTTR */}
                                        <td className="py-2.5 px-3 border-r border-[var(--border-color)] font-mono font-bold text-[var(--text-primary)]">
                                            <span className={`inline-flex items-center gap-1 ${parseFloat(row.avg) <= 4 ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400'}`}>
                                                <FaStopwatch size={10} /> {row.avg} Jam
                                            </span>
                                        </td>
                                ...
                                <tr className="bg-slate-100/60 dark:bg-slate-800/50 font-black border-b-2 border-[var(--border-color)]">
                                    ...
                                    <td className="py-2.5 px-3 border-r border-[var(--border-color)] font-mono text-[var(--text-muted)]">
                                        ≤ 4 Jam
                                    </td>
                                    ...
                                    {/* TOTAL IN SLA */}
                                    <td className="py-2.5 px-3 border-r border-[var(--border-color)] font-mono">
                                        {branchGroup.total.met > 0 ? (
                                            <button 
                                                onClick={() => handleOpenDetail('TOTAL MS', branchGroup.branch, 'MS', '', 'comply', branchGroup.total.met)}
                                                className="font-black text-emerald-600 dark:text-emerald-400 hover:bg-emerald-500/10 px-2 py-0.5 rounded transition cursor-pointer"
                                                title={`Lihat detail seluruh tiket In SLA MS (${branchGroup.branch})`}
                                            >
                                                {branchGroup.total.met}
                                            </button>
                                        ) : <span className="text-[var(--text-muted)] opacity-40">0</span>}
                                    </td>
                                    ...
                                    {/* TOTAL OVER SLA */}
                                    <td className="py-2.5 px-3 border-r border-[var(--border-color)] font-mono">
                                        {branchGroup.total.missed > 0 ? (
                                            <button 
                                                onClick={() => handleOpenDetail('TOTAL MS', branchGroup.branch, 'MS', '', 'not_comply', branchGroup.total.missed)}
                                                className="font-black text-rose-600 dark:text-rose-400 hover:bg-rose-500/10 px-2 py-0.5 rounded transition cursor-pointer"
                                                title={`Lihat detail seluruh tiket Over SLA MS (${branchGroup.branch})`}
                                            >
                                                {branchGroup.total.missed}
                                            </button>
                                        ) : <span className="text-[var(--text-muted)] opacity-40">0</span>}
                                    </td>

                                    {/* TOTAL REAL MTTR */}
                                    <td className="py-2.5 px-3 border-r border-[var(--border-color)] font-mono font-black">
                                        <span className={`inline-flex items-center gap-1 ${parseFloat(branchGroup.total.avg) <= 4 ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400'}`}>
                                            <FaStopwatch size={10} /> {branchGroup.total.avg} Jam
                                        </span>
                                    </td>
                                    ...
                                    <td className="py-2.5 px-4 font-mono">
                                        <span className={`inline-block px-2.5 py-1 rounded-md text-[11px] font-black font-mono ${
                                            parseFloat(branchGroup.total.avg) <= 4
                                            ? 'bg-emerald-500/15 text-emerald-600 dark:text-emerald-400 border border-emerald-500/30'
                                            : 'bg-rose-500/15 text-rose-600 dark:text-rose-400 border border-rose-500/30'
                                        }`}>
                                            {branchGroup.total.pct}%
                                        </span>
                                    </td>
                                </tr>
                            </React.Fragment>
                ...
                <div className="fixed inset-0 z-[99999] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-fadeIn">
                    <div className="w-full max-w-5xl bg-[var(--bg-surface)] rounded-2xl shadow-2xl border border-[var(--border-color)] overflow-hidden flex flex-col max-h-[85vh] my-auto relative">
                            ...
                            <div>
                                <div className="flex items-center gap-2.5">
                                    <h3 className="font-bold text-lg text-white tracking-tight">
                                        Detail Tiket MS: {modalState.title} {modalState.branch ? `(${modalState.branch})` : ''}
                                    </h3>
                                    <span className={`px-2.5 py-0.5 rounded-md text-xs font-black uppercase tracking-wider ${
                                        ...
                                        {modalState.type === 'comply' ? 'IN SLA (COMPLY)' : 'OVER SLA (NOT COMPLY)'}
                                    </span>
                                </div>
                        ...
                        <div className="p-4 bg-[var(--bg-base)] border-b border-[var(--border-color)] flex items-center justify-between gap-4 shrink-0">
                            <div className="relative flex-1">
                                ...
                                <input 
                                    ...
                                    className="w-full pl-9 pr-4 py-2 text-xs bg-[var(--bg-surface)] border border-[var(--border-color)] rounded-xl text-[var(--text-primary)] focus:outline-none focus:border-blue-500 transition"
                                />
                            </div>
                        </div>
                        ...
                        <div className="p-4 overflow-y-auto flex-1 custom-scrollbar">
                            {modalState.loading ? (
                                <div className="py-20 flex flex-col items-center justify-center gap-2 text-blue-600">
                                        ...
                                        <tbody className="divide-y divide-[var(--border-subtle)]">
                                            {filteredTickets.map((t) => (
                                                <tr key={t.id} className="hover:bg-[var(--bg-base)] transition-colors">
                                                    ...
                                                    <td className="p-3">
                                                        <span className="px-2 py-0.5 rounded bg-blue-50 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300 border border-blue-200 dark:border-blue-800/50 text-[10px] font-extrabold font-mono">
