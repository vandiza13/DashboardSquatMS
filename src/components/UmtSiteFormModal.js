...
export default function UmtSiteFormModal({ isOpen, onClose, onSuccess, initialData }) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const url = initialData ? `/api/umt-sites/${initialData.id}` : '/api/umt-sites';
                ...
                <div className="px-6 py-4 border-b border-[var(--border-color)] flex justify-between items-center bg-[var(--bg-surface-2)]">
                    <div>
                        <h3 className="font-bold text-lg text-[var(--text-primary)]">
                            {initialData ? `Edit Site UMT: ${formData.site_id}` : 'Tambah Site UMT Baru'}
                        </h3>
                    </div>
