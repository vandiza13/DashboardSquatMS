...
export async function GET(request) {
    try {
        ...
        const dataQuery = `SELECT * FROM public.fsi_sites${whereClause} ORDER BY created_at DESC LIMIT $${params.length + 1} OFFSET $${params.length + 2}`;
        const countQuery = `SELECT COUNT(*) as total FROM public.fsi_sites${whereClause}`;
...
export async function POST(request) {
    try {
        ...
        const insertQuery = `
            INSERT INTO public.fsi_sites (${columns.join(', ')}) 
            VALUES (${placeholders})
            RETURNING *
        `;
        ...
        // Sync ke Google Sheets (Provider: FSI)
        try {
            await appendSiteToSheet(createdSite, 'FSI');
        } catch (sheetError) {
            console.error("❌ Exception Google Sheet Sync FSI:", sheetError);
        }

        return NextResponse.json({ message: 'Site FSI berhasil ditambahkan', data: createdSite }, { status: 201 });

    } catch (error) {
        console.error("Create FSI Site Error:", error);
        if (error.code === '23505') {
