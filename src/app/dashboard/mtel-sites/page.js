...
import MtelSiteFormModal from '@/components/MtelSiteFormModal';
import MtelSiteDetailModal from '@/components/MtelSiteDetailModal';

export default function MtelSitesPage() {
    ...
    const fetchSites = useCallback(async () => {
        ...
        try {
            ...
            const res = await fetch(`/api/mtel-sites?${params}`);
            ...
            console.error("Error fetching MTEL Sites:", error);
        } finally {
            setLoading(false);
        }
    }, [page, search]);
    ...
    const handleDeleteClick = async (site) => {
        if (!confirm(`Hapus data Site MTEL ${site.site_id} secara permanen? Tindakan ini tidak dapat dibatalkan.`)) return;
        try {
            const res = await fetch(`/api/mtel-sites/${site.id}`, { method: 'DELETE' });
            const result = await res.json();
    ...
    if (userRole === null) {
        return (
            <div className="flex h-[80vh] w-full items-center justify-center flex-col gap-4">
                <div className="h-16 w-16 rounded-full border-4 border-[var(--border-color)] border-t-red-600 animate-spin"></div>
                ...
                <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                    <div>
                        <h2 className="text-xl md:text-2xl font-black text-[var(--text-primary)] flex items-center gap-2">
                            <span className="text-red-600"><FaGlobe /></span> Database Site MTEL
                        </h2>
                ...
                <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between rounded-2xl bg-[var(--bg-surface)] p-4 shadow-sm border border-[var(--border-color)]">
                    ...
                    <div className="flex bg-[var(--bg-base)] p-1 rounded-xl border border-[var(--border-color)] w-full md:w-auto">
                        <button
                            ...
                            className={`flex-1 md:flex-none flex items-center justify-center gap-2 px-6 py-2 rounded-lg text-xs font-bold transition-all ${viewMode === 'table' ? 'bg-red-600 text-white shadow-md' : 'text-[var(--text-muted)] hover:text-[var(--text-primary)]'}`}
                        >
                            <FaGlobe /> Data Tabel
                        </button>
                        <Link
                            href="/dashboard/mtel-sites/map"
                    ...
                    <div className="relative w-full md:w-auto">
                        ...
                        <input
                            type="text"
                            placeholder="Cari ID, Nama, STO..."
                            className="w-full md:w-72 rounded-xl border border-[var(--border-color)] bg-[var(--bg-base)] text-[var(--text-primary)] pl-10 pr-4 py-2.5 text-xs font-semibold focus:border-red-500 focus:outline-none transition-colors"
                            value={search}
                ...
                <div className="md:hidden space-y-4">
                    {loading ? (
                        ...
                        <div className="bg-[var(--bg-surface)] p-8 rounded-2xl border border-[var(--border-color)] text-center">
                            <EmptyState title="Tidak Ditemukan" message="Data site MTEL kosong atau pencarian Anda nihil." icon={FaGlobe} />
                        </div>
                    ) : (
                        sites.map((site) => (
                            <div key={site.id} className="bg-[var(--bg-surface)] p-4 rounded-2xl border border-[var(--border-color)] shadow-sm space-y-3 flex flex-col justify-between">
                                <div className="flex justify-between items-start border-b border-[var(--border-subtle)] pb-2">
                                    ...
                                    <span className="bg-red-50 dark:bg-red-900/30 text-red-600 dark:text-red-400 text-[10px] font-bold px-2 py-0.5 rounded border border-red-100 dark:border-red-800/50 uppercase">
                                        MTEL
                                    </span>
                                </div>
                        ...
                        <table className="w-full text-left text-sm">
                            ...
                            <tbody className="divide-y divide-[var(--border-subtle)] text-xs">
                                {loading ? (
                                    ...
                                    <tr>
                                        <td colSpan="4" className="p-12 text-center text-[var(--text-muted)] italic font-semibold">
                                            Data Site MTEL kosong atau pencarian Anda nihil.
                                        </td>
                                    </tr>
                                ) : (
                                    sites.map((site) => (
