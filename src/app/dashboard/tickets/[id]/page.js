...
import {
    ...
    FaHistory, FaExclamationCircle, FaTools, FaCopy, FaCheck
} from 'react-icons/fa';
    ...
    return (
        <div className="max-w-5xl mx-auto pb-20 animate-fade-in">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="space-y-6">
                    {/* BUTTON SALIN TR (HANYA SQUAT) */}
                    {ticket.category === 'SQUAT' && (
                        <button
                            onClick={handleCopyTR}
                            className={`w-full py-3 px-4 rounded-xl shadow-sm font-bold text-sm transition-all duration-200 transform hover:scale-[1.02] flex items-center justify-center gap-2 group ${
                                copied 
                                ? 'bg-green-600 text-white shadow-green-200' 
                                : 'bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white'
                            }`}
                        >
                            {copied ? <FaCheck className="animate-bounce" /> : <FaCopy className="group-hover:rotate-12 transition-transform" />}
