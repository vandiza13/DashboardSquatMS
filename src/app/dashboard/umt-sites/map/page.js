...
// Dynamic import UmtGisMap
const UmtGisMap = dynamic(() => import('@/components/UmtGisMap').then(mod => mod.default), {
    ssr: false,
    loading: () => (
        <div className="flex items-center justify-center h-full bg-[var(--bg-base)] rounded-2xl">
            <div className="text-center space-y-3">
                <div className="h-12 w-12 mx-auto rounded-full border-4 border-[var(--border-color)] border-t-orange-500 animate-spin" />
...
export default function UmtSitesMapPage() {
        ...
        <div className={`${isFullscreen ? 'fixed inset-0 z-[999] bg-[var(--bg-base)] flex flex-col' : 'space-y-0 pb-24 md:pb-0 w-full max-w-[100vw] overflow-hidden'} animate-[fadeIn_0.3s_ease_forwards]`}>
            ...
            <div className={`flex items-center justify-between gap-3 px-4 py-3 bg-[var(--bg-surface)] border-b border-[var(--border-color)] ${isFullscreen ? '' : 'rounded-t-2xl border border-[var(--border-color)]'} shrink-0`}>
                <div className="flex items-center gap-3 min-w-0">
                    <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-br from-orange-500 to-amber-600 text-white shadow-lg shadow-orange-500/25 shrink-0">
                ...
                <div className="flex items-center gap-2 shrink-0 overflow-x-auto no-scrollbar">
                    ...
                    <button
                        ...
                        className={`relative flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-bold border transition-all duration-200 whitespace-nowrap
                            ${isFilterOpen || search
                                ...
                                : 'bg-[var(--bg-base)] text-[var(--text-secondary)] border-[var(--border-color)] hover:border-orange-500/30'
                            }`}
                    >
                        <FaFilter size={10} />
                        <span className="hidden sm:inline">Filter</span>
                        {search && (
                            <span className="absolute -top-1 -right-1 w-4 h-4 bg-orange-500 text-white rounded-full text-[9px] flex items-center justify-center font-black">
                                1
                            </span>
                        )}
                    </button>
                    ...
                    <button
                        ...
                        className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-bold bg-[var(--bg-base)] text-[var(--text-secondary)] border border-[var(--border-color)] hover:border-orange-500/30 transition-all shrink-0"
                ...
                <div className={`overflow-hidden transition-all duration-300 ease-in-out bg-[var(--bg-surface)] ${isFilterOpen ? 'max-h-[300px] opacity-100 border-b border-[var(--border-color)]' : 'max-h-0 opacity-0'}`}>
                    <div className="p-4 space-y-3">
                        <div className="relative">
                            ...
                            <input type="text" placeholder="Cari Site ID, Nama, STO, Ring..." value={searchInput} onChange={(e) => setSearchInput(e.target.value)} className="w-full rounded-xl border border-[var(--border-color)] bg-[var(--bg-base)] text-[var(--text-primary)] pl-9 pr-4 py-2 text-xs font-semibold focus:border-orange-500 focus:outline-none" />
                            ...
                            return (
                                <div key={site.id || `${site.site_id}-${index}`} className="p-2.5 rounded-xl border border-[var(--border-color)] bg-[var(--bg-base)] hover:border-amber-300 transition-all">
                                    ...
                                    <div className="flex items-center gap-2">
                                        <span className="text-[9px] font-bold px-1.5 py-0.5 rounded-md text-white bg-orange-600">UMT</span>
                                    </div>
                                </div>
                            );
                        })}
